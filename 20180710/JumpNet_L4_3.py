...
class _SkipLinkLayer(nn.Sequential):
    ...
    def forward(self, x):
        ...
        return torch.cat([x, f_x, g_x, h_x, i_x], 1)
...
class _DenseBlock(nn.Sequential):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate):
        ...
        layer2 = _DenseLayer(num_input_features + 1 * growth_rate, growth_rate, bn_size, drop_rate)
        self.add_module('denselayer%d' % (2), layer2)

        jump_layer = _SkipLinkLayer(num_input_features + 2 * growth_rate, growth_rate, bn_size, drop_rate)
        self.add_module('jumplayer3_4_5_6', jump_layer)
