...
test_loader = torch.utils.data.DataLoader(dataset=test_dataset,
                                          ...
                                          shuffle=False,  #
                                          num_workers=4)
def prob_selNet(prev_Layer, Upper_Layer_arr, percentage): # fix first X, prev_Layer

    max_size = len(Upper_Layer_arr)
    if max_size == 0:
        return prev_Layer
    elif max_size == 1:
        return torch.cat([prev_Layer, Upper_Layer_arr[0]])
    ...
    for i in range(max_size):
        if i in active_indices:
            result_layer.append(Upper_Layer_arr[i])
        else:
            result_layer.append(torch.zeros(Upper_Layer_arr[i].size()))
...
class _halfLayer(nn.Sequential):

    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        super(_halfLayer, self).__init__()
    ...
    def forward(self, x):
        ...
        # prob_selNet(prev_Layer, Upper_Layer_arr, percentage)
        ...
        g_x = self.features2(torch.cat([x, f_x], 1))
        if self.drop_rate > 0:
            g_x = F.dropout(g_x, p=self.drop_rate, training=self.training)

        h_x = self.features2(torch.cat([x, g_x], 1))
        if self.drop_rate > 0:
            h_x = F.dropout(h_x, p=self.drop_rate, training=self.training)

        i_x = self.features3(torch.cat([x, f_x, h_x], 1))
        if self.drop_rate > 0:
            i_x = F.dropout(i_x, p=self.drop_rate, training=self.training)

        j_x = self.features3(torch.cat([x, g_x, i_x], 1))
        if self.drop_rate > 0:
            j_x = F.dropout(j_x, p=self.drop_rate, training=self.training)

        k_x = self.features4(torch.cat([x, f_x, h_x, j_x], 1))
        if self.drop_rate > 0:
            k_x = F.dropout(k_x, p=self.drop_rate, training=self.training)
...
class _DenseBlock(nn.Sequential):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate):
        ...
        halfLayer = _halfLayer(num_input_features, growth_rate, bn_size, drop_rate)
        self.add_module('jumplayer1_2_3_4_5_6', halfLayer)
