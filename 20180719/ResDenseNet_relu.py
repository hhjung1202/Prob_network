...
class _DenseLayer(nn.Sequential):
    ...
    def forward(self, x):
        ...
        new_features = new_features + x[:, self.growth_rate * -1:, :, :]
        new_features = self.relu(new_features)
        if self.drop_rate > 0:
            new_features = F.dropout(new_features, p=self.drop_rate, training=self.training)
        return torch.cat([x, new_features], 1)

class _DenseLayer_init(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        super(_DenseLayer_init, self).__init__()
        self.add_module('norm1', nn.BatchNorm2d(num_input_features)),
        self.add_module('relu1', nn.ReLU(inplace=True)),
        self.add_module('conv1', nn.Conv2d(num_input_features, bn_size *
                        growth_rate, kernel_size=1, stride=1, bias=False)),
    ...
    def forward(self, x):
        new_features = super(_DenseLayer_init, self).forward(x)
...
class _DenseBlock(nn.Sequential):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate):
        ...
        layer = _DenseLayer_init(num_input_features, growth_rate, bn_size, drop_rate)
        self.add_module('denselayer1', layer)
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(6,6),
