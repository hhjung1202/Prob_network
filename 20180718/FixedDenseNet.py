...
class _FixedDenseLayer(nn.Sequential):
    ...
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.feature_next = nn.Sequential(
                nn.BatchNorm2d(self.half_init_num + 6 * growth_rate),
                nn.ReLU(),
                nn.Conv2d(self.half_init_num + 6 * growth_rate, bn_size *
                            growth_rate, kernel_size=1, stride=1, bias=False),
        ...
        self.transition = nn.Sequential(
                nn.BatchNorm2d(num_input_features + 4 * growth_rate),
                nn.ReLU(),
                nn.Conv2d(num_input_features + 4 * growth_rate, self.half_init_num,
    ...
    def forward(self, x):
        ...
        j_x = self.feature4(torch.cat([x, f_x, g_x, h_x, i_x], 1))
        k_x = self.feature4(torch.cat([x, g_x, h_x, i_x, j_x], 1))

        x2 = self.transition(torch.cat([x, h_x, i_x, j_x, k_x], 1))
        ...
        a_x = self.feature_next(torch.cat([x2, f_x_c, g_x_c, h_x_c, i_x_c, j_x_c, k_x_c], 1))
        b_x = self.feature_next(torch.cat([x2, g_x_c, h_x_c, i_x_c, j_x_c, k_x_c, a_x], 1))
        c_x = self.feature_next(torch.cat([x2, h_x_c, i_x_c, j_x_c, k_x_c, a_x, b_x], 1))
        d_x = self.feature_next(torch.cat([x2, i_x_c, j_x_c, k_x_c, a_x, b_x, c_x], 1))
        e_x = self.feature_next(torch.cat([x2, j_x_c, k_x_c, a_x, b_x, c_x, d_x], 1))
        m_x = self.feature_next(torch.cat([x2, k_x_c, a_x, b_x, c_x, d_x, e_x], 1))
        n_x = self.feature_next(torch.cat([x2, a_x, b_x, c_x, d_x, e_x, m_x], 1))

        return torch.cat([x2, b_x, c_x, d_x, e_x, m_x, n_x], 1)
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=12, num_init_features=16, bn_size=4, drop_rate=0, num_classes=10):
