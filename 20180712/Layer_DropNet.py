...
def prob_selNet(prev_Layer, Upper_Layer_arr, percentage, training): # fix first X, prev_Layer
    ...
    if max_size == 0:
        ...
        return torch.cat([prev_Layer, Upper_Layer_arr[0]], 1)
    
    if percentage == 0 or not training or active_size == 0:
        return torch.cat([prev_Layer, torch.cat(Upper_Layer_arr, 1)], 1)

    active_indices = sorted(random.sample(range(max_size), active_size), reverse=True)

    result_layer = []
    for i in range(max_size):
        if i in active_indices:
            result_layer.append(torch.cuda.FloatTensor(Upper_Layer_arr[i]))
        else:
            result_layer.append(torch.cuda.FloatTensor(torch.zeros(Upper_Layer_arr[i].size()
                , dtype=Upper_Layer_arr[i].dtype, device=Upper_Layer_arr[i].device)))
    # result_layer = Upper_Layer_arr[active_indices]
    # torch.zeros(input.size())

    if len(result_layer) == 1:
        selected_layer = result_layer[0]
    else:
        selected_layer = torch.cat(result_layer, 1)
    # fill empty space by 0
    selected_layer = max_size / active_size * selected_layer

    # print(selected_layer)

    return torch.cat([prev_Layer, selected_layer], 1)

class _dropLayer(nn.Sequential):

    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        super(_dropLayer, self).__init__()
        ...
        #         nn.ReLU(),
        #         nn.Conv2d(num_input_features + i * growth_rate, bn_size *
        #                     growth_rate, kernel_size=1, stride=1, bias=False),
        ...
        self.feature4 = nn.Sequential(
                nn.BatchNorm2d(num_input_features + 4 * growth_rate),
                nn.ReLU(),
                nn.Conv2d(num_input_features + 4 * growth_rate, bn_size *
                            growth_rate, kernel_size=1, stride=1, bias=False),
        ...
        self.feature5 = nn.Sequential(
                nn.BatchNorm2d(num_input_features + 5 * growth_rate),
                nn.ReLU(),
                nn.Conv2d(num_input_features + 5 * growth_rate, bn_size *
                            growth_rate, kernel_size=1, stride=1, bias=False),
    ...
    def forward(self, x):
        ...
        prob_x = prob_selNet(x, [], self.drop_layer, self.training)
        f_x = self.feature0(prob_x)
        # print(self.feature0, self.features[0])
        # if self.drop_rate > 0:
        #     f_x = F.dropout(f_x, p=self.drop_rate, training=self.training)

        prob_f_x = prob_selNet(torch.cat([x, f_x], 1), [], self.drop_layer, self.training)
        g_x = self.feature1(prob_f_x)

        prob_g_x = prob_selNet(torch.cat([x, g_x], 1), [f_x], self.drop_layer, self.training)
        h_x = self.feature2(prob_g_x)

        prob_h_x = prob_selNet(torch.cat([x, h_x], 1), [f_x, g_x], self.drop_layer, self.training)
        i_x = self.feature3(prob_h_x)

        prob_i_x = prob_selNet(torch.cat([x, i_x], 1), [f_x, g_x, h_x], self.drop_layer, self.training)
        j_x = self.feature4(prob_i_x)

        prob_j_x = prob_selNet(torch.cat([x, j_x], 1), [f_x, g_x, h_x, i_x], self.drop_layer, self.training)
        k_x = self.feature5(prob_j_x)

        return torch.cat([x, f_x, g_x, h_x, i_x, j_x, k_x], 1)
...
class _DenseBlock(nn.Sequential):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate):
        ...
        dropLayer = _dropLayer(num_input_features, growth_rate, bn_size, drop_rate)
        self.add_module('droplayer1_2_3_4_5_6', dropLayer)
