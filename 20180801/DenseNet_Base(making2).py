...
class _mAttention(nn.Sequential):
    def __init__(self, num_input_features, compress_num = 0):
        ...
        for i in range(compress_num)
            self.add_module('att_conv%d' % (i), nn.Conv2d(num_input_features, num_input_features
                , kernel_size=3, stride=1, padding=1, groups=num_input_features, bias=False))
            self.add_module('att_pool%d' % (i), nn.AvgPool2d(kernel_size=2, stride=2))

        self.add_module('att_conv_last' , nn.Conv2d(num_input_features, num_input_features
                , kernel_size=5, stride=1, padding=1, groups=num_input_features, bias=False))
