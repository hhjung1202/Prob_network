...
class _mAttention(nn.Sequential):
    def __init__(self, num_input_features, compress_num = 0):
        super(_mAttention, self).__init__()
        for i in range(compress_num):
            self.add_module('att_conv%d' % (i+1), nn.Conv2d(num_input_features, num_input_features
                , kernel_size=3, stride=1, padding=1, groups=num_input_features, bias=False))
            self.add_module('att_pool%d' % (i+1), nn.AvgPool2d(kernel_size=2, stride=2))

        self.add_module('att_conv_last' , nn.Conv2d(num_input_features, num_input_features
                , kernel_size=5, stride=2, padding=1, groups=num_input_features, bias=False))

        self.add_module('att_sigmoid', nn.Sigmoid())

    def forward(self, x):
        Attention = super(_mAttention, self).forward(x)
        return Attention

class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, compress_num):
        super(_DenseLayer, self).__init__()

        self.compress_num = compress_num

        self.mAttention = _mAttention(num_input_features, compress_num)
...
class _DenseBlock(nn.Sequential):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, compress_num):
        super(_DenseBlock, self).__init__()
        for i in range(num_layers):
            input_channel = num_input_features + i * growth_rate
            layer = _DenseLayer(input_channel, growth_rate, bn_size, drop_rate, compress_num)
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=[8, 16, 32], block_config=(6,6,6),
        ...
        compress_num=[i for i in range(len(growth_rate),0,-1)] # [3,2,1]

        # Each denseblock
        num_features = num_init_features
        for i, num_layers in enumerate(block_config):
            block = _DenseBlock(num_layers=num_layers, num_input_features=num_features,
                                bn_size=bn_size, growth_rate=growth_rate[i], drop_rate=drop_rate, compress_num=compress_num[i])
