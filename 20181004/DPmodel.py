...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(6,6,6),
        ...
        if num_gate is 0:
            self.gate = _Gate(channels=num_features, reduction=4, count=block_config[-1]+1)
        elif num_gate is 1:
            self.gate = _Gate(channels=num_features, reduction=2, count=block_config[-1]+1)
        elif num_gate is 2:
            self.gate = _Gate2(channels=num_features, reduction=16, count=block_config[-1]+1)
        elif num_gate is 3:
            self.gate = _Gate2(channels=num_features, reduction=24, count=block_config[-1]+1)
        elif num_gate is 4:
            self.gate = _Gate2(channels=num_features, reduction=32, count=block_config[-1]+1)
